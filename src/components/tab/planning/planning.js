(() => {
  return {
    ...
    computed: {
      ...
      dayText(){
        return this.selected ? bbn.dt(this.selected).format('dddd	DD MMMM YYYY') : '';
      },
      calendars(){
        if ( this.isYearMode ){
    ...
    methods: {
          ...
          this.post(this.root + 'data/planning/day', {day: day, filters: this.currentFilters}, (d) => {
            if ( d.data ){
              d.data = d.data.map((e, i) => {
                e.hour = bbn.dt(e.end).diff(bbn.dt(e.start), 'minutes') / 60;
          ...
          bbn.fn.each(d.events, (e, i) => {
            ...
            if ( idx === -1 ){
              tmp.push({
                ...
                hour: bbn.dt(e.end).diff(bbn.dt(e.start), 'minutes')
              });
            }
            else {
              tmp[idx].hour += bbn.dt(e.end).diff(bbn.dt(e.start), 'minutes');
            }
          });
      ...
      nextYear(){
        if ( this.isYearMode ){
          this.currentYear = bbn.dt(this.currentYear, 'YYYY').add(1, 'Y').format('YYYY');
          bbn.fn.each(this.calendars, c => c.next(true));
        }
      },
      prevYear(){
        if ( this.isYearMode ){
          this.currentYear = bbn.dt(this.currentYear, 'YYYY').subtract(1, 'Y').format('YYYY');
